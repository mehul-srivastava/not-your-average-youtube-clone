...
const Navbar = async () => {
  ...
  return (
    <nav className="flex h-full w-full items-center justify-between border-b border-b-gray-700/70 px-5 text-white">
      <Link href="/auth/identity?redirectTo=/" className="flex items-center gap-4">
      ...
      <div className="flex items-center gap-4">
        ...
        {session && <Image width={36} height={36} src={session.user?.image!} className="h-9 w-9 rounded-md" alt="user image" />}
      </div>
    </nav>
  );
};
